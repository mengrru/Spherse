...
│   │   └── src/
    │   │       ├── index.ts              # createMultiProjectServer()，创建 logger、Fastify 实例并注册 ProjectRegistry；注册 Host 校验 / 认证制 CORS / auth 三个 onRequest hook
    │   │       ├── cors.ts              # 认证制 CORS hook（preflight 反射放行，token 有效才设 ACAO）
