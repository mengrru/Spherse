...
│   │       │   ├── chat/                 # 对话 feature；replica/ 放三区副本纯状态机（durable 有序集/run 尾块/intents/notices/derive 视图派生/sync 三层追赶），runtime/ 放 WS/心跳/重连传输壳与 registry，replica-store.ts 为薄 store（rAF 批量归约），model/ 放事件解析、历史投影、turn 分组、重试/撤回判定纯函数，hooks/ 放 UI hooks，lib/ 放聚合/diff/format-time 纯函数，utils/ 放图片压缩（compress-image）；根目录保留页面组件、运行时 context、chat 专属类型与附件 UI（AttachmentBar/MessageAttachments）
