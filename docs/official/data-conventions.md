...
Agent 聊天窗口主题存放于同目录的 `theme.css`。该文件由 Agent Dialog 的“主题”标签页编辑，新建流程初始为空白（用户从零编写；`@spherse/presets` 的 `agent-theme-template.css` 仅作为参考物料保留，不参与初始化）。文件不存在时读取结果为空字符串，聊天窗口使用全局默认样式。
