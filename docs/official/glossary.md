...
| 自定义侧边面板（custom side panel） | ProjectPanel 的可切换视图：按项目配置的 HTML（`project.yaml` 的 `sidePanel.path`）经透明 iframe 占满项目面板；与指整个左侧栏的 side panel（ActivityBar + ProjectPanel）无关 | [data-conventions.md](data-conventions.md) |
