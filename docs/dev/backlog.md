...
- [ ] **chat 错误分类兜底 TRANSIENT 的收敛**：服务端 `classify-run-error.ts` 与 renderer `classify-error.ts` 对未知错误都兜底 TRANSIENT——程序性错误（TypeError、内部 bug）也会触发最多 2 次自动重试，可能掩盖缺陷；若失败发生在副作用型工具执行后，retry-last 会重新执行工具。方向：不可重试类（ValidationError 文案等）前置排除，或自动重试日志中标记诱因分类，双端规则保持一致。
