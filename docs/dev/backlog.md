...
- [x] **Landing page**：新建 `@spherse/landing` package，使用 GitHub Pages 部署项目介绍页。包含 Hero 区（标题+下载按钮）、轮播区（4 张 APP 截图 + 锚定按钮对齐 avatar + 主题色随切换改变 + 5 秒自动播放）、特性卡片（4 张，点击打开浮层展示截图）、即将到来的功能（跨 session 记忆 + MCP）、三语切换（复用 `@spherse/i18n` 类型）。参见 `docs/dev/features/2026-07-12-landing-page/design.md`
