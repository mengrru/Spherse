...
| 面板定位 | header 外包一层 `relative shrink-0` wrapper，面板 `absolute top-full inset-x-0 z-30`（对齐 side-panel 抽屉的层叠量级，压过 MessageList 内 sticky/滚动按钮），高度 `h-[35dvh]`（绝对定位下百分比高度会错误解析到 wrapper，用 dvh 近似"35%"，移动 PWA 全屏下即窗口高的 35%）。**实施调整**：放弃「常驻挂载 + translate-y + inert」改为「打开时挂载 + 关闭即卸载」——`useContentFile` 无 enabled 参数，常驻挂载会在关闭态以空路径发起无效请求；关闭方向无动画为可接受代价，切换链接时组件不重挂载故动画不重播。**入场动画**：`clip-path` inset 自顶向下展开（`quick-link-unfold` keyframes，`styles.css`），从 header 底部划出、不遮盖 header（`slide-in-from-top` 会先平移到 header 区域再落下，视觉盖住 header，弃用）；`motion-reduce:animate-none` 降级 |
| 面板关闭 | 面板不自带 header 与关闭按钮（按钮已有选中/反选形态，面板标题栏冗余）；再点同一按钮 toggle 关闭；点其他链接切换内容（不重播动画）；quickLinks 刷新后若 `activeQuickLink` 不在新列表则置 null（配置侧删除链接的边界） |
