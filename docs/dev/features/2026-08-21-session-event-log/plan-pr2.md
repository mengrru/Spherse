...
- **legacy 会话 fork**：`forkSession` 检测 needsMigration → 先 `migrateLegacySession` 再 fork（一步完成）
...
- 测试：contract 测试 + routes 单测（branch/recall/migrate、409/404/422 错误路径）
...
- **迁移 CTA**：`needsMigration` 会话——打开时历史正常渲染（legacy 读路径）、composer 位置替换为「升级此会话」按钮 → 调 migrate → 成功后原地变为可输入；session 列表 needsMigration 会话带标记；旧会话点「分支/撤回」= 自动迁移 + 动作一步完成（toast 提示已升级）
- i18n：zh-CN 基准注释 + zh-TW/en
- 测试：chat-history 投影（撤回后隐藏）、迁移 CTA 组件、lineage badge；结构测试按现有模式
