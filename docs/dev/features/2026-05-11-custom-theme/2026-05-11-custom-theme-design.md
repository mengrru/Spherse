...
支持用户通过 `.pi/theme.css` 文件覆盖默认 CSS 变量，实现主题定制。

## 加载时机

- 打开项目后，renderer 检查项目目录下是否存在 `.pi/theme.css`
- 存在则注入到 `<head>`，不存在则跳过

## 实现方式

1. Renderer 获取当前项目路径
2. 检查 `.pi/theme.css` 是否存在
3. 如存在，注入 `<link rel="stylesheet" href="path/.pi/theme.css?t={mtime}">` 到 `<head>`
