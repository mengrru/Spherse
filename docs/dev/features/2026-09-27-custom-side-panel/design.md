...
| 渲染切换 | `ProjectPanel` 顶部读 store 激活态 + query 解析结果：`active && resolvedPath` → 渲染 `<CustomSidePanel key={projectId} path={path} />` **替换**默认三段内容；否则渲染既有默认内容（AgentSessionList + UserFilePanel + SkillPanel）。path 被清除/不可达时自动回落默认内容，激活态保留（休眠，重新配置后自动恢复显示）。`key={projectId}`：ProjectPanel 不随项目切换重挂（WelcomePage 的干净切换依赖页面级 key，见 `WelcomePagePage.tsx` 注释），须在组件上补 key 防旧项目的 reloadKey/pathRef/iframe 残留（同 iframe 换 src 是导航而非重挂）。外层 ContextMenu 与 `aside` 骨架不变 |
