...
| 关闭拦截 | `electron/tray.ts` 的 `attachCloseToTray(win)` 仅挂在主窗口 `close` 上：`isQuitting()` → 放行；窗口**已隐藏**时收到 `close`（非用户操作，典型为 NSIS 安装覆盖时 `taskkill` 发 `WM_CLOSE`）→ `preventDefault()` + `app.quit()` 走优雅退出；`closeToTray` 启用 → `preventDefault()` 并隐藏（macOS 全屏时先 `setFullScreen(false)`，等 `leave-full-screen` 再隐藏，避免残留黑色 Space），macOS 额外 `app.dock?.hide()`；否则放行 |
...
| 托盘图标资源 | 新增 `packages/desktop/resources/tray/`：macOS 模板图 `trayTemplate.png`（16px）/ `trayTemplate@2x.png`（32px），由 `spherse-icon.svg` 去白底、纯黑透明渲染，系统随明暗自动反色；Windows/Linux 用 `tray.png`（16px）/ `tray@2x.png`（32px），为现有图标缩放。`nativeImage.createFromPath` 自动拾取 `@2x`。`electron-builder.yml` `extraResources` 增 `resources/tray → tray`；运行时 `app.isPackaged ? process.resourcesPath/tray : app.getAppPath()/resources/tray` |
...
- app 设置组件：`capabilities.tray=false` 时不渲染开关
- E2E `close-to-tray.spec.ts`：默认关闭窗口 → 隐藏且进程存活，`second-instance` 唤回；关闭开关后关闭窗口 → 进程退出
