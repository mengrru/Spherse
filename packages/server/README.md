...
`src/` 目录按域而非按层组织：内聚的 WS 域收进域目录（当前仅 `chat/`——hub/channel/projector/ws-chat/classify-run-error，对外只经 `chat/index.ts` 导出 `handleChatWebSocket` + `ChatSessionHub`，channel/projector 是内部实现）；不设 `services/`、`ws/` 等通用层目录。REST 端点一律进 `routes/`，跨 route 共享的有状态编排留在顶层（`registry.ts`、`marketplace.ts`、`ws-bus.ts`）。
