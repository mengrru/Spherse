...
function keyed(messages: ChatMessage[]): KeyedMessage[] {
  return messages.map((message, index) => ({ key: `k${index}`, message }));
}
...
describe("groupTurns", () => {
  it("returns plain groups for manual messages", () => {
    ...
    const groups = groupTurns(keyed(messages));
    expect(groups).toEqual([
      { kind: "plain", item: { message: messages[0], key: "k0", index: 0 } },
      { kind: "plain", item: { message: messages[1], key: "k1", index: 1 } },
    ]);
  });
  ...
  it("returns an empty array for empty messages", () => {
    expect(groupTurns(keyed([]))).toEqual([]);
  });
});
