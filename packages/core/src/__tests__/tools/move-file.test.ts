...
import { createTempProject, cleanupDir, writeFile, readFile, pathExists, permissivePolicy } from "../helpers.js";
...
describe("createMoveFileTool", () => {
  ...
  it("moves a file", async () => {
    ...
    const tool = createMoveFileTool(projectRoot, mutex, permissivePolicy(projectRoot));
  ...
  it("returns error when source is denied by access policy", async () => {
    ...
    const tool = createMoveFileTool(projectRoot, mutex, () => llmAccessPolicy(projectRoot, ["secrets"]));
