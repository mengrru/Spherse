...
import { createTempProject, cleanupDir, writeFile, readFile, ensureDir, pathExists } from "../helpers.js";
...
describe("createCopyFileTool", () => {
  ...
  it("copies a file", async () => {
    ...
    const tool = createCopyFileTool(projectRoot, mutex, () => createAiFileAccessPolicy(projectRoot, []));
  ...
  it("returns error when source is denied by AI access policy", async () => {
    ...
    const tool = createCopyFileTool(projectRoot, mutex, () => createAiFileAccessPolicy(projectRoot, ["secrets"]));
